...
plotHeatmapui <- function(id) {
    ...
    tagList(
        seuratToolsBox(
            ...
            selectizeInput(ns("dendroSelect"), "Clustering algorithm or metadata categorical variable for column arrangement", choices = NULL, selected = NULL, multiple = TRUE),
    ...
    tagList(
        seuratToolsBox(
            ...
            conditionalPanel(
                ...
                selectizeInput(ns("varSelectGroup"), "Categorical variable used to define groups", choices = NULL, multiple = FALSE),
            ...
            conditionalPanel(
                ...
                actionButton(
                    ns("saveClust1"),
                    "Save to Custom Group 1"
                ), actionButton(
                    ns("saveClust2"),
                    "Save to Custom Group 2"
                )
            ),
...
plotReadCount <- function(input, output, session, seu, plot_types) {
    ...
    output$metavarui <- renderUI({
        req(seu())
        shiny::selectInput(ns("metavar"), "Variable for y-axis (must be numeric variable)",
            choices = purrr::flatten_chr(plot_types()), selected = c("nCount_Gene"), multiple = FALSE
        )
    })
...
monocleui <- function(id) {
    ...
    tagList(
        ...
        seuratToolsBox(
            ...
            selectizeInput(ns("plottype1"), "Variable to Plot", choices = c(Louvain = "louvain"), selected = "cluster_names_Res_1.6", multiple = TRUE),
            selectizeInput(ns("customFeature1"), "Gene expression by which to color the plot",
        ...
        seuratToolsBox(
            title = "Embedding Plot",
            selectizeInput(ns("plottype2"), "Variable to Plot", choices = c(Louvain = "louvain"), selected = "cluster_names_Res_1.6", multiple = TRUE),
            selectizeInput(ns("customFeature2"), "Gene expression by which to color the plot",
                choices = NULL, multiple = FALSE
            ),
        ...
        #     # actionButton(ns("plotMonocleHeatmap"), "Plot Heatmap"),
        #     downloadButton(ns("downloadPlot"), "Download Heatmap"),
...
monocle <- function(input, output, session, seu, plot_types, featureType,
    ...
    # markermarker
    w <- waiter::Waiter$new(ns("ptimeGenesDT"),
                            html = waiter::spin_loaders(id = 1, color = "black", style = "position:relative;margin:auto;"),
                            color = waiter::transparent(.5)
    )
    ...
    observeEvent(input$calcCDS, {
        ...
        updateSelectizeInput(session, "plottype1", selected = "cluster_names_Res_1.6", choices = myplot_types())
        updateSelectizeInput(session, "customFeature1", choices = rownames(cds), server = TRUE)
        updateSelectizeInput(session, "plottype2", selected = "batch", choices = myplot_types())
    ...
    monocle_heatmap <- reactive({
        ...
        monocle_module_heatmap(cds_rvs$traj, rownames(heatmap_genes), cells = input$rootCells ,input$cdsResolution, collapse_rows = input$heatmapRows, group.by = input$colAnnoVar)
    }) %>%
...
techInfo <- function(input, output, session, seu) {
    ...
    observe({
        ...
        output$sample_info_general <- renderText({
            ...
            #for package version
            info_packages_raw <- misc()$experiment$session_info
            package_info_lines <- info_packages_raw[[1]][13:184]
            info_packages <- c()
            for (i in 1:length(package_info_lines)) {
                info_packages <- paste(info_packages, "<br>", package_info_lines[i])
            }
            paste0(
                info,
                ...
                info_R,
                "</pre>",
                "<li><strong>Pacakge versions:</strong></li>",
                "</ul>",
                "<pre>",
                info_packages,
                "</pre>"
...
plotCoverage_UI <- function(id) {
    ...
    tagList(
        seuratToolsBox(
            ...
            selectizeInput(ns("varSelect"), "Categorical variable used to define groups", choices = NULL, multiple = FALSE),
